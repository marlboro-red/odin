//! The built-in web status dashboard: a single embedded HTML page plus the read-only JSON API
//! it polls. Reads are unauthenticated (like `/metrics`); the page's approve/reject buttons
//! HMAC-sign in the browser and call the existing signed `/approve`, so the server takes no new
//! mutating route. Projections live here; the axum handlers + routing live in `webhook.rs`.

use odin_core::{ArtifactName, RunState};
use serde::Serialize;

/// A run as the dashboard list shows it — the lighter projection (no diff).
#[derive(Serialize)]
pub(crate) struct RunRow {
    run_id: String,
    workflow: String,
    /// The run status, lowercase serde string (`succeeded`, `awaiting_approval`, …).
    status: String,
    updated_at: String,
    steps: Vec<StepRow>,
    /// For an `awaiting_approval` run: the gate step + its message, so the page can render the
    /// approve/reject controls. `None` for any other status.
    gate: Option<GateRow>,
}
...
struct StepRow {
    ...
    /// Why the step failed/was skipped (first line is enough for the UI).
    error: Option<String>,
}
...
/// One run's full detail: the row plus the captured diff and run-level error.
#[derive(Serialize)]
...
/// Serializes any value to its canonical lowercase string (e.g. a `RunStatus`/`StepStatus`
/// serde tag), so the projection never drifts from the wire representation.
...
fn steps(state: &RunState) -> Vec<StepRow> {
    state
        .steps
        .iter()
        .map(|(id, st)| StepRow {
...
/// The gate currently parked at `awaiting_approval`, if any (matched by the serde tag so it
/// can't drift from `StepStatus::AwaitingApproval`).
fn gate(state: &RunState) -> Option<GateRow> {
    state
        .steps
        .iter()
...
/// Projects a stored run into the list row (no diff).
pub(crate) fn project_row(state: &RunState) -> RunRow {
    RunRow {
...
/// Projects a stored run into the full detail (row + diff + run-level error).
pub(crate) fn project_detail(state: &RunState) -> RunDetail {
    let diff = state
        .artifacts
